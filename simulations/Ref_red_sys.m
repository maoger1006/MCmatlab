...
%   for iRun = 1:n
%      model = runMonteCarlo(model);
%      R4(i,iRun) = model.MC.lightCollector.image*100;
%      
...
model = plot(model,'MC');
model = plot(model,'G');
% writematrix(R2,'Results.xlsx','Sheet',1,'Range' , 'A4');
...
%   end
  ...
  mediaProperties(j).mus = 256.2;  % Jacques "Optical properties of biological tissues: a review" eq. 2
  mediaProperties(j).g   = 0.9;   % Scattering anisotropy
  mediaProperties(j).n   = 1.3;
...
%   end
  ...
  mediaProperties(j).mua = 0.104;
  mediaProperties(j).mus = 62.0 ;
...
%   end
  ...
  mediaProperties(j).mua = 0.816;
  mediaProperties(j).mus = 86.1;  % Jacques "Optical properties of biological tissues: a review" eq. 2
...
%   end
  ...
  mediaProperties(j).mua = 0.351;
  mediaProperties(j).mus = 344.5;  % Jacques "Optical properties of biological tissues: a review" eq. 2
