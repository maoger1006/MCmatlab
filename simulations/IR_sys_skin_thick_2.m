...
model.G.Lz                = 1.305+0.0475*1.5; % [cm] z size of simulation cuboid
...
writematrix(R1,'Results.xlsx','Sheet',2,'Range' , 'A10')

%% Geometry function(s) (see readme for details)
function M = geometryDefinition(X,Y,Z,parameters)
  % Blood vessel example:
  corneum_thick = 0.002*1.375;
  epidermis_thick = 0.025*1.375;
  papillary_thick = 0.01*1.375;
  upper_dermis = 0.008*1.375;
  reticular_dermis = 0.02*1.375;
  deep_dermis = 0.03*1.375;
  muscle_thick = 1;
  fat_thick = 0.055;
  boneradius  = 0.2;
  bonedepth = 0.5;
...
%   end
  ...
  mediaProperties(j).mua = 0.0017;
  mediaProperties(j).mus = 0.542 ;
...
%   end
  ...
  mediaProperties(j).mua = 0.00401;
  mediaProperties(j).mus = 0.581;  % Jacques "Optical properties of biological tissues: a review" eq. 2
...
%   end
  ...
  mediaProperties(j).mua = 0.00457;
  mediaProperties(j).mus = 2.47;  % Jacques "Optical properties of biological tissues: a review" eq. 2
...
%   end
  ...
  mediaProperties(j).mus = 1.568;  % Jacques "Optical properties of biological tissues: a review" eq. 2
  mediaProperties(j).g   = 0.92;   % Scattering anisotropy
  mediaProperties(j).n   = 1.3;
end