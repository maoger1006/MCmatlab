...
model.G.Lz                = 1.302; % [cm] z size of simulation cuboid (1.305)
...
model.MC.nPhotonsRequested  = 1e+08; % [min] Time duration of the simulation
...
model.MC.wavelength               = 940; % [nm] Excitation wavelength, used for determination of optical properties for excitation light
...
model.MC.lightSource.focalPlaneIntensityDistribution.radialWidth = .0135; % [cm] Radial focal plane 1/e^2 radius if top-hat or Gaussian or half-width of the full distribution if custom
...
model.MC.lightSource.yFocus       = -0.3; % [cm] y position of focus
...
model.MC.lightCollector.y         = 0.3; % [cm] y position
...
model.MC.lightCollector.diam      = .6; % [cm] Diameter of the light collector aperture. For an ideal thin lens, this is 2*f*tan(asin(NA)).
...
%   for iRun = 1:n
%      model = runMonteCarlo(model);
%      R2(i,iRun) = model.MC.lightCollector.image*100;
%      
...
for i = 1:3
%          model.MC.lightCollector.y         = i/5-0.3; % [cm] y position
         model.MC.lightCollector.fieldSize = 0.05*i +0.05; % [cm] Field Size of the imaging system (diameter of area in object plane that gets imaged). Only used for finite f.
  for iRun = 1:n
...
% writematrix(R2,'Results.xlsx','Sheet',1,'Range' , 'A2');
...
function M = geometryDefinition(X,Y,Z,parameters)
  ...
  M = ones(size(X));  %air
  M(Z > air_thick) = 2;  % stratum corneum
  M(Z > air_thick + corneum_thick) = 3; % epidermis
  M(Z > air_thick + corneum_thick + epidermis_thick) = 4; % papillary
  M(Z > air_thick + corneum_thick + epidermis_thick + papillary_thick) = 5; %upper_dermis
  M(Z > air_thick + corneum_thick + epidermis_thick + papillary_thick + upper_dermis ) = 6; % reticular_dermis
  M(Z > air_thick + corneum_thick + epidermis_thick + papillary_thick + upper_dermis + reticular_dermis ) = 7;%deep_dermis
  M(Z > air_thick + corneum_thick + epidermis_thick + papillary_thick + upper_dermis + reticular_dermis + deep_dermis ) = 8;%fat
  M(Z > air_thick + corneum_thick + epidermis_thick + papillary_thick + upper_dermis + reticular_dermis + deep_dermis + fat_thick)= 9;%muscle
  M(Z > air_thick + corneum_thick + epidermis_thick + papillary_thick + upper_dermis + reticular_dermis + deep_dermis + fat_thick + muscle_thick)= 8;%fat                                                                                               
  M(Z > air_thick + corneum_thick + epidermis_thick + papillary_thick + upper_dermis + reticular_dermis + deep_dermis + 2*fat_thick + muscle_thick)= 7;%deep_dermis
  M(Z > air_thick + corneum_thick + epidermis_thick + papillary_thick + upper_dermis + reticular_dermis + 2*deep_dermis + 2*fat_thick + muscle_thick)= 6;% reticular_dermis
  M(Z > air_thick + corneum_thick + epidermis_thick + papillary_thick + upper_dermis + 2*reticular_dermis + 2*deep_dermis + 2*fat_thick + muscle_thick)= 5;%upper_dermis
  M(Z > air_thick + corneum_thick + epidermis_thick + papillary_thick + 2*upper_dermis + 2*reticular_dermis + 2*deep_dermis + 2*fat_thick + muscle_thick)= 4;% papillary
  M(Z > air_thick + corneum_thick + epidermis_thick + 2*papillary_thick + 2*upper_dermis + 2*reticular_dermis + 2*deep_dermis + 2*fat_thick + muscle_thick)= 3; % epidermis
  M(Z > air_thick + corneum_thick + 2*epidermis_thick + 2*papillary_thick + 2*upper_dermis + 2*reticular_dermis + 2*deep_dermis + 2*fat_thick + muscle_thick)= 2;% stratum corneum
  M(X.^2 + (Z - (corneum_thick+bonedepth)).^2 < boneradius^2) = 10;%bone
...
%   mua_base = 7.84*(10^7)*(model.MC.wavelength )^(-3.255);
  ...
  mua_art = Sa*0.65 + (1-Sa)*0.43;
  mua_ven = Sv*0.65 + (1-Sv)*0.43;
  mua_w =  0.2674;
...
%   end
  ...
  mediaProperties(j).mus = 156.8;  % Jacques "Optical properties of biological tissues: a review" eq. 2
  mediaProperties(j).g   = 0.9;   % Scattering anisotropy
  mediaProperties(j).n   = 1.3;
...
%   end
  ...
  mediaProperties(j).mua = 0.17;
  mediaProperties(j).mus = 54.2 ;
...
%   end
  ...
  mediaProperties(j).mua = 0.401;
  mediaProperties(j).mus = 58.1;  % Jacques "Optical properties of biological tissues: a review" eq. 2
...
%   end
  ...
  mediaProperties(j).mua = 0.457;
  mediaProperties(j).mus = 247;  % Jacques "Optical properties of biological tissues: a review" eq. 2
  mediaProperties(j).g   = 0.9;   % Scattering anisotropy
  mediaProperties(j).n   = 1.3;
